...
public class MainActivity extends FragmentActivity implements View.OnClickListener {
    ...
    private void init () {
        ...
        centerButton.setCenterButtonListener (new IwfuCenterButton.CenterButtonListener () {
            ...
            public void openCenterButton () {
                // 当按钮打开,弹出三个子按钮
                toggleSonButtons (sonLinearLayout1, centerButton.getX () + 10, frameLayout.getY (), 180f, 180f);
                toggleSonButtons (sonLinearLayout2, centerButton.getX () + 10, frameLayout.getY (), 0f, 250f);
                toggleSonButtons (sonLinearLayout3, centerButton.getX () + 10, frameLayout.getY (), -180f, 180f);
            }

            @Override
            public void closeCenterButton () {
                // 当按钮关闭,回收三个子按钮
                toggleSonButtons (sonLinearLayout1, sonLinearLayout1.getX (), sonLinearLayout1.getY (), -180f, -180f);
                toggleSonButtons (sonLinearLayout2, sonLinearLayout2.getX (), sonLinearLayout2.getY (), 0f, -250f);
                toggleSonButtons (sonLinearLayout3, sonLinearLayout3.getX (), sonLinearLayout3.getY (), 180f, -180f);
            }
        });
    }
     ...
     * @param deltaY Y变化
     */
    private void toggleSonButtons (final View target, float startX, float startY, float deltaX, float deltaY) {
        target.setVisibility (View.VISIBLE);
        AnimatorSet set = new AnimatorSet ();

        ObjectAnimator transXAnimator = ObjectAnimator.ofFloat (target, "translationX", startX, startX - deltaX);
        ObjectAnimator transYAnimator = ObjectAnimator.ofFloat (target, "translationY", startY, startY - deltaY);
     ...
     *
     * @param context 上下文
     * @param title   子按钮下的文本提示
     * @param resId   子按钮图标
     * @return 新建的子按钮
     */
    private LinearLayout addSonLinearLayout (Context context, String title, int resId) {
        ...
        contentMain.addView (sonLinearLayout);
